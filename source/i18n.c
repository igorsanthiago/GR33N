...
static const struct {
	...
	{ "Maquina lista en %u ms (%s:%u es relleno). Falta WebRTC: mira el log.",
	  "Máquina pronta em %u ms (%s:%u de espera). Conectando WebRTC..." },
	...
	{ "Izq/Der Cambiar    Arriba/Abajo Mover    ",
	  "Esq/Dir Alterar    Cima/Baixo Mover    " },
	{ "Stick der. Descripcion    /\\ Favorito    ",
	  "Analógico Dir. Rolar    /\\ Favorito    " },
	{ "%s Ficha    [] %s    /\\ Favorito    ",
	  "%s Detalhes    [] %s    /\\ Favorito    " },
	...
	{ "L3 + R3", "L3 + R3" },
	{ "SELECT + %s / SELECT + START", "SELECT + %s / SELECT + START" },
	{ "%s / SELECT + START", "%s / SELECT + START" },
	...
	{ "Entra con tu cuenta de Microsoft. La consola te da un codigo corto y tu lo escribes en el movil: la contrasena no pasa por aqui en ningun momento, que es justo para lo que sirve este metodo. Lo que se guarda en la PS3 es un permiso que caduca y que puedes retirar cuando quieras desde tu cuenta.",
	  "Entre com sua conta Microsoft. O console exibe um código curto para digitar no celular ou PC: sua senha nunca passa pelo PS3. Fica salvo apenas um token de autorização que você pode revogar quando quiser." },
	...
	{ "El idioma de GR33N. El de los juegos es el ajuste de abajo. Solo hay dos porque la fuente esta dibujada a mano, pixel a pixel: llega para el castellano y el ingles, y anadir ruso o japones seria dibujar un alfabeto entero.",
	  "O idioma da interface do GR33N. O idioma dos jogos é configurado logo abaixo." },
